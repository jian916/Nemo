...
function Enable64kHairstyle() {
	...
	//Step e_1a - Find address of Format String
	var code = "\xBE\xC7\xBC\xBC\xBB\xE7\xB8\xAE\\%s\\%s_\xB9\xE8\xC6\xB2\xBF\xC2\xB6\xF3\xC0\xCE%s.%s"; // "악세사리\%s\%s_배틀온라인%s.%s"
	var offset = exe.findString(code, RAW);
	if (offset === -1)
		return "Failed in Step e_1 - String not found";

	//Step e_1b - Change the 2nd %s to %u
	exe.replace(offset + code.length - 4, "75", PTYPE_HEX);

	//Step e_1c - Find the string reference
	offset = exe.findCode("68" + exe.Raw2Rva(offset).packToHex(4), PTYPE_HEX, false);
	if (offset === -1)
		return "Failed in Step e_1 - String reference missing";

	//Step e_2a - Move offset to previous instruction which should be an PUSH reg
	var regNum = exe.fetchUByte(offset - 1) - 0x50;
	if (regNum < 0 || regNum > 7)
		return "Failed in Step e_2 - Unknown instruction before reference";

	//Step e_2b - Extract the register code used in the third last PUSH reg32 before the reference
	var regNum = exe.fetchUByte(offset - 0xB) - 0x50;
	if (regNum < 0 || regNum > 7)
		return "Failed in Step e_2 - Missing Reg PUSH";

	regc = (0x45 | (regNum << 3)).packToHex(1);
	regd = (0xF8 + regNum).packToHex(1);
	rege = (0xB8 + regNum).packToHex(1);

	//Step e_2c - Now look for the location where it is assigned. Dont remove the AB at the end, the code size is used later.
	code =
	    " 83 7D AB 10"          //CMP DWORD PTR SS:[EBP-y], 10 ; y is unknown
	  + " 8D" + regc + " AB"    //LEA reg32, [EBP-z] ; z = y+5*4
	  + " 0F 43" + regc + " AB" //CMOVAE reg32, DWORD PTR SS:[EBP-z]
	...
	//Step e_2d - Save the offset2 and code size (We need to NOP out the excess)
	var assignOffset = offset2;
	var csize = code.hexlength();

	//Step e_3a - Find the start of the function (has a common signature like many others)
	code =
	    " 6A FF"             //PUSH -1
	  + " 68 AB AB AB 00"    //PUSH value
	  + " 64 A1 00 00 00 00" //MOV EAX, FS:[0]
	...
	if (offset === -1) { // 2018 +
		offset = exe.find(code, PTYPE_HEX, true, "\xAB", offset2 - 0x380, offset2);
	}
	if (offset === -1)
		return "Failed in Step e_3 - Function start missing";

	offset += code.hexlength();

	//Step e_3b - Get the Stack offset w.r.t. ESP/EBP for Arg.5
	var arg5Dist = 5*4; //for the 5 PUSHes of the arguments
	arg5Dist += 4; //Account for the PUSH EBP in the beginning

	//Step e_3c - Set return address
	var retadd = assignOffset + csize ;

	//Step e_3d - Find free space to inject our code
	var free = exe.findZeros(0x14 + 4);
	if (free === -1)
		return "Failed in Step e_3 - Not enough free space";

	retadd = exe.Raw2Rva(retadd) - exe.Raw2Rva(free + 0x14);

	//Step e_3e - Prep code to change assignment (hairstyle index instead of the string)
	code =
	    " 8B" + regc + arg5Dist.packToHex(1)          //MOV reg32_A, DWORD PTR SS:[EBP + arg5Dist]; ARG.5
	  + " 8B" + ((regNum << 3) | regNum).packToHex(1) //MOV reg32_A, DWORD PTR DS:[reg32_A]
	  + " 83" + regd + "1D"                           //CMP reg32_A, 1D (29)
	  + " 76 05"                                      //JBE two line
	  + rege + " 01 00 00 00"                         //MOV reg32_A, 1
	  + " E9" + retadd.packToHex(4)                   //JMP retaddress
	;
	var asize = code.hexlength();

	//Step e_3f - Insert code
	exe.insert(free, asize+4, code, PTYPE_HEX);

	//Step e_3g - Prep code for jump
	var offsetj = exe.Raw2Rva(free) - exe.Raw2Rva(assignOffset + 5);
	code = " E9" + offsetj.packToHex(4);
	code += " 90".repeat(csize - code.hexlength()); //Fill rest with NOPs

	//Step e_3h - Replace the original at assignOffset
	exe.replace(assignOffset, code, PTYPE_HEX);

	//Step e_4a - Find the string table fetchers
	code =
	    " 8B AB AB AB AB 00" //MOV reg32_A, DWORD PTR DS:[addr]
	  + " 8B AB 00"          //MOV reg32_B, DWORD PTR DS:[EBP]
	  + " 8B 14"             //MOV EDX, DWORD PTR DS:[reg32_B * 4 + reg32_A]
	...
	if (offsets.length === 0)
		return "Failed in Step e_4 - Table fetchers missing";

	//Step e_4b - Remove the reg32_B * 4 from all the matches
	for (var i = 0; i < offsets.length; i++) {
		offset2 = offsets[i] + code.hexlength();
		exe.replaceWord(offset2 - 1, 0x9010 + (exe.fetchByte(offset2) & 0x7));
	}
